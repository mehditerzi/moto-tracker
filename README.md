...
│                  │         │              └── React PWA (static)     │
└──────────────────┘         │                                         │
                             │  ollama   (vision model, default gemma4:26b) │
                             │  sqlite   (./data/app.db)               │
                             └─────────────────────────────────────────┘
```
...
5. pull the configured Ollama vision model (`gemma4:26b` by default)
...
| Bikes | Multiple vehicles per user (motorcycle **or** car), archive, plate auto-uppercased, optional make/model/year/km/color |
...
| OCR | Tesseract → text-LLM → vision fallback, per-user queue with real cancellation, JSON parser tolerant of prose / code fences, Turkish date normalization, deterministic plate/date backstops, auto-apply when confidence ≥ 0.7 and bike matches by plate |
| Vehicle catalog | Bundled make/model catalog (vPIC + curated Turkish-market overlay + API-Ninjas) covering motorcycles **and** cars; OCR canonicalizes extracted make/model against it, and the UI offers searchable make/model/color/provider dropdowns to minimize typing |
...
| `OLLAMA_VISION_MODEL` | no | default `gemma4:26b`; use `gemma4:e4b` for ~2-3× faster scans, `gemma4:e2b` for fastest |
| `OCR_AUTO_APPLY_THRESHOLD` | no | default `0.7` — minimum confidence for auto-applying a dated_item |
| `OCR_CONCURRENCY` | no | default `2` — max documents OCR'd in parallel across users (per-user is always serial) |
| `API_NINJAS_KEY` | no | dev-only; used by `scripts/fetch-moto-catalog.mjs` to enrich the bundled make/model catalog. Not needed at runtime |
...
**OCR pipeline.** `POST /api/documents` (multipart) → `sharp` re-encodes the image (max 1024 px, grayscale, q=80 JPEG, EXIF rotation respected) → row inserted with `ocr_status='pending'` → the worker picks it up (serialized **per user**, up to `OCR_CONCURRENCY` documents in parallel across users, with a shared per-document deadline that actually cancels a hung Tesseract/Ollama). The pipeline runs Tesseract first, then a text LLM, falling back to the vision model when the text result is unreadable/uncertain. The response is parsed against a Zod schema that tolerates code fences, leading/trailing prose, and `dd.mm.yyyy` Turkish date formats. Deterministic backstops then recover a plate/date the model dropped from the raw OCR text, and the make/model are canonicalized against the bundled vehicle catalog. If `doc_type ∈ {sigorta,kasko,muayene}` and confidence ≥ threshold and the bike matches by plate (or single-bike user), a `dated_item` is inserted automatically with `source_document_id` set and `needs_review=1`. Otherwise the document is left parsed for the user to confirm.

**Vehicle catalog.** A bundled TS module (`apps/api/src/db/seed/vehicleCatalog.generated.ts`) holds motorcycle + car makes/models. It's seeded into `vehicle_make`/`vehicle_model` at startup (idempotent, keyed on a fingerprint) and powers both OCR canonicalization and the `/api/catalog/{makes,models}` search endpoints behind the UI dropdowns.

Regenerate with `node scripts/fetch-moto-catalog.mjs` — **no API key needed**. Sources, in order of weight:

| source | supplies | limits |
| --- | --- | --- |
| curated Turkish-market overlay (in the script) | **all car models**, most motorcycle models, brand aliases, and the popularity order | hand-maintained |
| NHTSA vPIC (free, no key) | make breadth only | US registrations — knows nothing about Renault/Dacia/Peugeot/Citroën/Opel/Škoda/Seat/Togg models, and calls a Fiat Egea a "Tipo". Model harvest is off by default (`--vpic-models`) |
| API-Ninjas (`API_NINJAS_KEY`, dev-only) | tops up makes the overlay leaves under 8 models | free tier caps at 30 rows/make with no offset; car corpus is US trim data |

Flags: `--offline` (overlay only, no network), `--fresh` (discard the previous build's harvest), `--audit` (report without writing), `--vpic-models`. Regeneration is **non-destructive** by default — the previous build's models are carried forward, so running without an `API_NINJAS_KEY` cannot delete what a key-holding run harvested. The script also audits for model pairs close enough to confuse the OCR fuzzy matcher and prints them.

Overlay list order is data: it becomes a 1–1000 popularity rank (`vehicle_make.pop_car`/`pop_moto`, `vehicle_model.popularity`, migration 026) that orders the autocomplete, so the car make dropdown opens on Fiat/Renault/Volkswagen rather than alphabetically.

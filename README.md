...
**Vehicle catalog.** A bundled TS module (`apps/api/src/db/seed/vehicleCatalog.generated.ts`) holds motorcycle + car makes/models, regenerated by `node scripts/fetch-moto-catalog.mjs` (NHTSA vPIC + a curated Turkish-market overlay + API-Ninjas model names; set `API_NINJAS_KEY` to enrich models). It's seeded into `vehicle_make`/`vehicle_model` at startup (idempotent) and powers both OCR canonicalization and the `/api/catalog/{makes,models}` search endpoints behind the UI dropdowns.
